...
def remove_rules(lines: list[str]) -> list[str]:
...
def update_generated_config(path: Path, want_enable: bool) -> None:
    if not path.exists():
        return
    lines = remove_rules(split_lines(path.read_text(encoding="utf-8")))
    if want_enable:
        insert_at = None
        for index, line in enumerate(lines):
            if line.strip() == "rules:":
...
def main() -> int:
    ...
    try:
        for target in targets:
            if target.exists():
                backup = target.with_name(target.name + f".bak-bilibili-toggle-{timestamp}")
    ...
    print(f"已{action}B站走代理规则。")
    print(reload_message)
    print()
    print("已处理的持久规则文件：")
    print(rules_file)
    print()
    print("提示：如果B站客户端已有旧连接，退出并重新打开B站后会完全按新规则连接。")
    return 0
