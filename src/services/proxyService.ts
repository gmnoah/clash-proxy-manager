...
// 服务规则定义（内嵌，不依赖外部文件）
// ============================================================
const SERVICE_RULES: Record<string, { name: string; rules: string[] }> = {
  qishui: {
    name: "汽水音乐",
    rules: [
...
// ============================================================
// 内嵌的 Python 脚本（不依赖外部文件）
// ============================================================
const PROXY_TOGGLE_SCRIPT = `
...
const DEFAULT_SERVICE_TEMPLATES = [
  // 音乐
  { id: "qishui", name: "汽水音乐", icon: "🎵", category: "音乐", enabled: false },
  { id: "qqmusic", name: "QQ音乐", icon: "🎶", category: "音乐", enabled: false },
  { id: "netease", name: "网易云音乐", icon: "☁️", category: "音乐", enabled: false },
  { id: "kugou", name: "酷狗音乐", icon: "🐶", category: "音乐", enabled: false },
  { id: "kuwo", name: "酷我音乐", icon: "🎧", category: "音乐", enabled: false },
  { id: "migu", name: "咪咕音乐/视频", icon: "🎤", category: "音乐", enabled: false },
  // 视频
  { id: "bilibili", name: "B站", icon: "📺", category: "视频", enabled: false },
  { id: "tencentvideo", name: "腾讯视频", icon: "🎬", category: "视频", enabled: false },
  { id: "iqiyi", name: "爱奇艺", icon: "🥝", category: "视频", enabled: false },
  { id: "youku", name: "优酷", icon: "🔶", category: "视频", enabled: false },
  { id: "mangotv", name: "芒果TV", icon: "🥭", category: "视频", enabled: false },
  { id: "xigua", name: "西瓜视频", icon: "🍉", category: "视频", enabled: false },
  // 短视频/直播
  { id: "douyin", name: "抖音", icon: "📱", category: "短视频/直播", enabled: false },
  { id: "huya", name: "虎牙直播", icon: "🐯", category: "短视频/直播", enabled: false },
  { id: "douyu", name: "斗鱼直播", icon: "🐟", category: "短视频/直播", enabled: false },
  // 有声
  { id: "ximalaya", name: "喜马拉雅", icon: "🏔️", category: "有声", enabled: false },
];
...
export async function loadServices(): Promise<ProxyService[]> {
  try {
    ...
    if (!(await exists(configPath))) {
      ...
      await saveServices(DEFAULT_SERVICE_TEMPLATES);
    ...
    const newServices = DEFAULT_SERVICE_TEMPLATES.filter((s) => !existingIds.has(s.id));
...
function generateScript(): string {
  // 构建 SERVICES 字典的 Python 字面量格式
  const entries = Object.entries(SERVICE_RULES)
    .map(([id, svc]) => {
      const rulesStr = svc.rules.map((r) => `            "${r}"`).join(",\n");
      return `    "${id}": {
        "name": "${svc.name}",
        "rules": [
${rulesStr}
  ...
  const servicesPyDict = `{\n${entries}\n}`;

  return PROXY_TOGGLE_SCRIPT.replace("{{SERVICES_JSON}}", servicesPyDict);
}

/**
 * 切换代理状态
 * 直接通过 stdin 把 Python 脚本传给 python3 执行，不依赖任何外部文件
 */
export async function toggleProxy(
  ...
  try {
    ...
    // 生成完整的 Python 脚本
    const pythonScript = generateScript();

    console.log(`执行代理切换: ${serviceId} ${mode}`);

    // 通过 sh -c 把脚本传给 python3 的 stdin 执行
    // 这样完全不依赖任何外部文件
    const cmd = `python3 - '${serviceId}' '${mode}' <<'PYEOF'\n${pythonScript}\nPYEOF`;

    const output = await Command.create("/bin/sh", ["-c", cmd]).execute();
